...
<div class="w-full min-h-[calc(100vh-100px)]	text-left margin-auto">
	<h1 class="text-center text-3xl py-3 sm:text-4xl sm:py-4">
		...
		{#if emoji === 0}
			<span
				class="inline-block cursor-default origin-[67%_67%] hover:animate-wave"
				>👋</span
			>
		{:else if emoji === 1}
			<span
				class="inline-block cursor-default origin-[50%_80%] hover:animate-peace"
				>✌️</span
			>
		{:else}
			<span
				class="inline-block cursor-default origin-[50%_80%] hover:animate-horns"
				>🤘</span
			>
		{/if}
	...
	<p class="mt-16">
		...
		optimization testing, and Bayesian prob. I also make a mean custom Slack
		emoji.
	</p>
</div>
